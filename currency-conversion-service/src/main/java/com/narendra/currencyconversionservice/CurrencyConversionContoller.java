...
public class CurrencyConversionContoller {
	...
	public CurrencyConversionBean converCurrency(@PathVariable String from,
		...
		response.setTotalCalculatedAmount(quantity.multiply(response.getExchangeRate()));
		return new CurrencyConversionBean(response.getId(), from, to, response.getExchangeRate(), quantity,
				response.getTotalCalculatedAmount(), response.getPortNumber());
	}
